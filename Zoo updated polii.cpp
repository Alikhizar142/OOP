...
class Elephent : public Animal
{
    int weight;
    public:
    Elephent(string s="",string c="",int w=0);
    void display();
};

Elephent::Elephent(string s,string c,int w):Animal(s,c)
{
    weight=w;
}
void Elephent:: display()
{
    cout<<"Im a Elephant my name is "<<name<<endl;
    cout<<"Im a Elephant my color is "<<color<<endl;
    cout<<"Im a Elephant my weight is"<<weight<<endl;

}



void displayall(Animal *a[],int n)
{
    for(int i=0;i<n;i++)
    {
        a[i]->display();
...
{   int n;
    ...
    cin>>n;
    Animal *A[n];
    A[0]=new Deer("Deer","White",true);
    A[1]=new Zebra("Zebra","Black","striped");
    A[2]=new Elephent("Elephant","Grey",220);
    displayall(A,n);
    for(int i=0;i<n;i++)
    {
       delete A[i];
